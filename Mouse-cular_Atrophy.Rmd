...
ctl_d.integrated <- FindClusters(ctl_d.integrated, resolution = 2, verbose = FALSE)
```
...
FeaturePlot(ctl_d.integrated, features = c("Myh1", "Myh2", "Myh4", "Myh7"))
# Features Plot for cluster identification 
# FeaturePlot(ctl_d.integrated, features = c("Myh7"))
# Heatmap 
ctl_d.integrated.markers %>% group_by(cluster) %>% top_n(n = 2, wt = avg_log2FC) -> top2
DoHeatmap(ctl_d.integrated, features = top2$gene) + NoLegend() 
```
...
```{r}
# clustree(sct.data, prefix = seurat_clusters)
```

...
annotations <- GetGRangesFromEnsDb(ensdb = EnsDb.Mmusculus.v79) # Install "EnsDb.Mmusculus.v79"
...
x.multi <- SCTransform(x.multi, verbose = FALSE)
```
...
x.multi <- FindClusters(x.multi, graph.name = "wsnn", algorithm = 3, verbose = FALSE)
```

# Visualisation 

```{r}
umap.snrna <- DimPlot(pbmc, reduction = "umap.rna", group.by = "celltype", label = TRUE, label.size = 2.5, repel = TRUE) + ggtitle("RNA")
umap.atac <- DimPlot(pbmc, reduction = "umap.atac", group.by = "celltype", label = TRUE, label.size = 2.5, repel = TRUE) + ggtitle("ATAC")
umap.wnn <- DimPlot(pbmc, reduction = "wnn.umap", group.by = "celltype", label = TRUE, label.size = 2.5, repel = TRUE) + ggtitle("WNN")
umap.snrna + umap.atac + umap.wnn & NoLegend() & theme(plot.title = element_text(hjust = 0.5))
```

...
wnn_analysis <- function(input_multi.data){
  # Trimming 
  input_multi.data <- subset(x = input_multi.data, subset = nCount_ATAC < 7e4 & nCount_ATAC > 5e3 & nCount_RNA < 25000 & nCount_RNA > 1000 & percent.mt < 20)
  # Normalization 
...
frag.file <- here("data","Ctl-D", "for_SIGNAC", "atac_fragments.tsv.gz") # Don't forget to change this part, if there is an issue with this part try the following bash script : gzip -d atac_fragments.tsv.gz | bgzip | tabix -p bed
...
_Work in Progress_ 
Do a for loop for merge dataset and a quick analysis for a single data set

```{r}
ctl_d.combined <- merge(x = x.multi, y = y.multi, add.cell.ids = c("CTL", "D"), project = "Ctl_D_Analysis")
# Plot the number of nuclei
table(ctl_d.combined$orig.ident)
# Split the data into a list 
ctl_d.list <- SplitObject(object = ctl_d.combined, split.by = "orig.ident")
...
ctl_d.integrated <- RunTFIDF(ctl_d.integrated)
ctl_d.integrated <- FindTopFeatures(ctl_d.integrated, min.cutoff = 'q0')
ctl_d.integrated <- RunSVD(ctl_d.integrated)
ctl_d.integrated <- RunUMAP(ctl_d.integrated, reduction = 'lsi', dims = 2:50, reduction.name = "umap.atac", reduction.key = "atacUMAP_")
```
...
ctl_d.integrated <- FindMultiModalNeighbors(ctl_d.integrated, reduction.list = list("pca", "lsi"), dims.list = list(1:50, 2:50))
ctl_d.integrated <- RunUMAP(ctl_d.integrated, nn.name = "weighted.nn", reduction.name = "wnn.umap", reduction.key = "wnnUMAP_")
ctl_d.integrated <- FindClusters(ctl_d.integrated, graph.name = "wsnn", algorithm = 3, verbose = FALSE)
```

# 4.6 Visualisation 

```{r}
umap.snrna <- DimPlot(ctl_d.integrated, reduction = "umap.rna", label = TRUE) + ggtitle("RNA")
umap.atac <- DimPlot(ctl_d.integrated, reduction = "umap.atac", label = TRUE) + ggtitle("ATAC")
umap.wnn <- DimPlot(ctl_d.integrated, reduction = "wnn.umap", label = TRUE) + ggtitle("WNN")
umap.snrna
umap.atac
umap.wnn
```

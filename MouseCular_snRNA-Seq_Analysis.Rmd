...
```{r}
counts <- Read10X(data.dir = "data/Ctl-D/outs/filtered_feature_bc_matrix") # Read10X() function read in the matrix and rename its row names and col names by gene symbols and cell barcodes, respectively.
...
/!\ There is no one-size-fits-all filtering criteria, as the normal ranges of these metrics can vary dramatically from one experiment to another, depending on sample origin as well as reagents and sequencing depths. 
...
## 4. Feature selection
Single-cell analysis has a major advantage over bulk RNA-seq in its ability to explore cellular heterogeneity and identify distinct cell groups. However, proper feature selection is necessary as genes with low expression levels or similar expression across all cells may dilute differences between cell groups.
...
Pros: 
- SCT resolve the introduction of the zero-inflation artifact using a regularized negative binomial regression model to normalize scRNA-seq data.
Cons: 
- Slow
- The normalized expression measurements is data-dependent. Indeed, in SCT, information from the other cells in the same data set is involved during normalization. This potentially introduces problems when multiple data sets need to be compared. 
- SCT involve random sampling to speed up the computation. Thus the result is slightly different every time, even if it is applied to the same data set.
...
The batch effect may be small, so no integration is necessary. As a result, data sets can be examined first by simply merging them.
...
Running the IntegrateData() function creates a new assay called `integrated`, where the batch-corrected expression matrix is stored. The uncorrected values are store in the `RNA` assay). 
The default assay of the resulted Seurat object is automatically set to `integrated`, but you can switch to the other one by using `DefaultAssay(seurat.rna) <- "RNA"`.

Then the UMAP embedding and clustering should be done with the `integrated` assay but the corrected values are no longer very reliable as the quantitative measure of gene expression. Thus, it is recommended that for the other analysis such as cluster marker identification and visualization, to use the uncorrected expression values instead using the `RNA` assay
...
### 8.2-B Using Harmony (RECOMMENDED)
Harmony uses fuzzy clustering to assign each cell to multiple groups. For each cluster, a correction factor is calculated for each data set to bring the centroid of the cluster closer to the global centroid. Since each cell is represented as a combination of multiple clusters, a cell-specific correction factor is determined by averaging the correction factors of the clusters to which the cell belongs, weighted by the cluster assignment ratio. This iterative process continues until convergence is achieved or iteration limits are reached. _(https://doi.org/10.1038/s41592-019-0619-0)_
...
Each dataset is renamed, and then grouped under different categories, here according to muscle or status (healthy or denervated).
...
It is then necessary to identify groups of cells in an unbiased way via clustering. 
A network of cells with k nearest neighbors is generated. Each cell is first connected to the cells with the shortest distances, based on their corresponding PC values. Only cell pairs that are neighbors of each other are considered connected. The proportion of shared neighbors between each pair of cells is then calculated and used to describe the strength of the connection between two cells. Weak connections are eliminated. The result is the network of shared nearest neighbors (SNN).

Next, Louvain's community identification algorithm is applied to the network to search for communities in the network, i.e. groups of cells that cells in the same group tend to connect to each other, while connections between cells in different groups are few. (The Leiden algorithm seems to perform better and faster according to the latest literature benchmarks)

```{r}
seurat.rna <- FindNeighbors(seurat.rna, dims = 1:ncol(Embeddings(seurat.rna)), reduction = "harmony")
seurat.rna <- FindClusters(seurat.rna, resolution = 0.3, algorithm = 1) # algorithm = 1 -> Lauvain / 4 -> Leiden
```

A step with Clustree can be added to know the decomposition of each cluster according to the resolution in order to choose the resolution.
The aim is to choose a resolution that is relatively stable and consistent with the number of cell populations expected in the data set. 
...
Add a `gtype` category for 'general type', grouping each cell type into its own cluster 
...
Add a `gtype_muscular` category to group all non-muscle cell types into a single cluster (Non-Muscular)
...
region_enrichment <- data.frame(region = levels(seurat.rna$gtype),
                                t(sapply(levels(seurat.rna$gtype), function(gtype)
                                  freq_fisher(conditions = factor(seurat.rna$status, levels=c("Control","Denervated")),
...
region_enrichment <- data.frame(region_enrichment,
                                ...
                                freq_glm_aov(samples = seurat.rna$orig.ident,
                                             conditions = factor(seurat.rna$status, levels=c("Control","Denervated")),
...
meta <- seurat_transcriptome@meta.data %>% mutate(status = factor(status, levels=c("Control","Denervated")))
...
```{r}
# Convert you Seurat Object 
# OPTIONAL: Do subset (only myonuclei + NMJ + MTJ)
# seurat.rna <- subset(seurat.rna, subset = RNA_snn_res.0.6 %in% c(0,2,3,5,6,7,11,14)) 
...
contrast_tbl = tibble(contrast = c("Control-Denervated","Denervated-Control"), 
                      group = c("Control","Denervated"))
```

```{r}
# OPTIONAL: Subset our data and only keep cells from patients that belong to the groups of interest 
...
ligand_activities_targets_DEgenes = suppressMessages(suppressWarnings(get_ligand_activities_targets_DEgenes(
  ...
  logFC_threshold = 0.75, # 0.75 for paired / 0.5 otherwise
...
```{r}
# ERROR ?!
lr_target_prior_cor = lr_target_prior_cor_inference(prioritization_tables$group_prioritization_tbl$receiver %>% unique(), abundance_expression_info, celltype_de, grouping_tbl, prioritization_tables, ligand_target_matrix, logFC_threshold = 0.5, p_val_threshold = 0.05, p_val_adj = F, top_n_LR = 250)
```
...
multinichenet_output = make_lite_output(multinichenet_output, top_n_LR = NA)
...
prioritized_tbl_oi_D_50 = get_top_n_lr_pairs(multinichenet_output$prioritization_tables, 50, groups_oi = 'Denervated')

plot_oi = make_sample_lr_prod_activity_plots(multinichenet_output$prioritization_tables, prioritized_tbl_oi_D_50) # PLOT
plot_oi
```

```{r}
# With D_IIb as receiver 
prioritized_tbl_oi_D_50 = get_top_n_lr_pairs(multinichenet_output$prioritization_tables, 50, groups_oi = 'Denervated', receivers_oi = "D_IIb")

plot_oi = make_sample_lr_prod_activity_plots(multinichenet_output$prioritization_tables, prioritized_tbl_oi_D_50)
plot_oi

prioritized_tbl_oi_D_50 = get_top_n_lr_pairs(multinichenet_output$prioritization_tables, 50, groups_oi = 'Denervated', senders_oi = "D_IIb")
...
```{r}
group_to_test = "Denervated"
receivers_to_test = "D_IIb"

prioritized_tbl_oi_D_10 = get_top_n_lr_pairs(multinichenet_output$prioritization_tables, 10, groups_oi = group_to_test, receivers_oi = receivers_to_test)
combined_plot = make_ligand_activity_target_plot(group_to_test, receivers_to_test, prioritized_tbl_oi_D_10, multinichenet_output$prioritization_tables, multinichenet_output$ligand_activities_targets_DEgenes, contrast_tbl, multinichenet_output$grouping_tbl, abundance_expression_info$celltype_info, ligand_target_matrix, plot_legend = FALSE)
combined_plot
```
...
lr_target_prior_cor_filtered = multinichenet_output$lr_target_prior_cor %>% inner_join(ligand_activities_targets_DEgenes$ligand_activities %>% distinct(ligand, target, direction_regulation, contrast)) %>% inner_join(contrast_tbl) %>% filter(group == group_to_test, receiver == receivers_to_test)
...
lr_target_correlation_plot = make_lr_target_correlation_plot(multinichenet_output$prioritization_tables, prioritized_tbl_oi,  lr_target_prior_cor_filtered , multinichenet_output$grouping_tbl, abundance_expression_info$celltype_info, receiver_oi,plot_legend = FALSE)
lr_target_correlation_plot$combined_plot
```

Visualize the expression correlation for a selected LR pair and their targets
...
sender = "D_IIb"
receiver = "IIb"
lr_target_scatter_plot = make_lr_target_scatter_plot(multinichenet_output$prioritization_tables, ligand, receptor, sender, receiver, abundance_expression_info$celltype_info, multinichenet_output$grouping_tbl, lr_target_prior_cor_filtered)
